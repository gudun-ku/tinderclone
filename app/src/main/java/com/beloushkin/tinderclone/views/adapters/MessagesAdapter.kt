    ...
    override fun onCreateViewHolder(parent: ViewGroup, itemViewType: Int): MessageViewHolder {
        if(itemViewType == MESSAGE_CURRENT_USER)
            return MessageViewHolder(
                LayoutInflater.from(parent.context).inflate(R.layout.item_current_user_message,
                    parent,false)
            )
        else
            return MessageViewHolder(
                LayoutInflater.from(parent.context).inflate(R.layout.item_other_user_message,
                    parent,false)
            )
